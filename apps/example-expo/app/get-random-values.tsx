...
export default function Index() {
  ...
  const runBasic = useCallback(() => {
    try {
      ...
      if (!getRandomValues)
        throw new Error('globalThis.crypto.getRandomValues missing')
  ...
  const runTypedArrays = useCallback(() => {
    try {
      ...
      for (const [name, make] of cases) {
        ...
        if (getRandomValues(array) !== (array as never)) {
  ...
  const runRejections = useCallback(() => {
    try {
      ...
      for (const [name, make] of cases) {
        try {
          getRandomValues(make() as Uint8Array)
        } catch (e) {
  ...
  const runQuota = useCallback(() => {
    try {
      ...
      try {
        getRandomValues(new Uint16Array(40_000) as never)
      ...
      getRandomValues(new Uint8Array(65_536))
  ...
  const runBigInt = useCallback(() => {
    try {
      ...
      if (getRandomValues(array as never) !== (array as never)) {
        throw new Error('did not return same reference')
      }
  ...
  const runUniqueness = useCallback(() => {
    try {
      ...
      const a = toHex(getRandomValues(new Uint8Array(16)))
      const b = toHex(getRandomValues(new Uint8Array(16)))
