...
describe('randomUUID', () => {
  ...
  it('uses ExpoCrypto.randomUUID (SDK 48+) and lowercases it', () => {
    nativeSpy.mockImplementation(() => {
      throw new Error(
        "TurboModuleRegistry.getEnforcing(...): 'RandomUuid' could not be found. Verify that a module by this name is registered in the native binary.",
      )
    })
  ...
  it('throws the getEnforcing error when no source is available', () => {
    nativeSpy.mockImplementation(() => {
      throw new Error(
        "TurboModuleRegistry.getEnforcing(...): 'RandomUuid' could not be found. Verify that a module by this name is registered in the native binary.",
      )
    })
    expect(() => randomUUID()).toThrow(/could not be found/)
  })
